...
def _compare_values(assertion: Assertion, actual: Any) -> None:
    ...
    if op == AssertionOperator.EQ:
    ...
    elif op in (AssertionOperator.LT, AssertionOperator.LE, AssertionOperator.GT, AssertionOperator.GE):
        ...
        try:
            ...
            raise AssertionFailure(f"Cannot numerically compare '{actual}' and '{expected}'", expected, actual)
        ...
        if not isinstance(expected, list):
            raise AssertionFailure(f"'in' operator requires a list, got {type(expected).__name__}", expected, actual)
        if actual not in expected:
            raise AssertionFailure(f"Expected {actual} to be in {expected}", expected, actual)
            
    elif op == AssertionOperator.CONTAINS:
        if actual is None or expected is None:
            raise AssertionFailure(f"Cannot use 'contains' with null", expected, actual)
        try:
            if expected not in actual:
                raise AssertionFailure(f"Expected '{actual}' to contain '{expected}'", expected, actual)
        except TypeError:
            raise AssertionFailure(f"Cannot check if {type(actual).__name__} contains {type(expected).__name__}", expected, actual)
            
    elif op == AssertionOperator.MATCHES:
        if not isinstance(actual, str) or not isinstance(expected, str):
            raise AssertionFailure(f"'matches' operator requires strings", expected, actual)
        try:
            if not re.search(expected, actual):
                raise AssertionFailure(f"Expected '{actual}' to match regex '{expected}'", expected, actual)
        ...
        if expected == "string" and not isinstance(actual, str):
            raise AssertionFailure(f"Expected type string, got {type(actual).__name__}", expected, actual)
        elif expected == "number" and not isinstance(actual, (int, float)):
            raise AssertionFailure(f"Expected type number, got {type(actual).__name__}", expected, actual)
        elif expected == "boolean" and not isinstance(actual, bool):
            raise AssertionFailure(f"Expected type boolean, got {type(actual).__name__}", expected, actual)
        elif expected == "object" and not isinstance(actual, dict):
            raise AssertionFailure(f"Expected type object, got {type(actual).__name__}", expected, actual)
        elif expected == "array" and not isinstance(actual, list):
            raise AssertionFailure(f"Expected type array, got {type(actual).__name__}", expected, actual)
